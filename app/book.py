...
def search():
    if request.method == 'POST':
        ...
        query = "SELECT b.*, c.filename FROM books b INNER JOIN covers c ON b.cover_id= c.id_covers WHERE title LIKE '%{}%'".format(keyword)
        with db.connection.cursor(named_tuple = True) as cursor:
            cursor.execute(query)
            results= cursor.fetchall()
          
        return render_template('books/search.html', results=results,bgs=load_book_genres(),genres = load_genres())
